...
# Paths to files for unsupervised optical flow training
TRAIN_UNSUPERVISED_OPTICAL_FLOW_IMAGES_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_images.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_SPARSE_DEPTH_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_sparse_depth.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_GROUND_TRUTH_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_ground_truth.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_FOCAL_LENGTH_BASELINE_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_focal_length_baseline.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_INTRINSICS_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_intrinsics.txt')

TRAIN_UNSUPERVISED_OPTICAL_FLOW_IMAGES_LEFT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_images_left.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_IMAGES_RIGHT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_images_right.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_SPARSE_DEPTH_LEFT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_sparse_depth_left.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_SPARSE_DEPTH_RIGHT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_sparse_depth_right.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_GROUND_TRUTH_LEFT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_ground_truth_left.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_GROUND_TRUTH_RIGHT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_ground_truth_right.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_FOCAL_LENGTH_BASELINE_LEFT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_focal_length_baseline_left.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_FOCAL_LENGTH_BASELINE_RIGHT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_focal_length_baseline_right.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_INTRINSICS_LEFT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_intrinsics_left.txt')
TRAIN_UNSUPERVISED_OPTICAL_FLOW_INTRINSICS_RIGHT_FILEPATH = os.path.join(
    TRAIN_UNSUPERVISED_REF_DIRPATH, 'kitti_train_optical_flow_intrinsics_right.txt')
...
def process_frame_optical_flow(inputs):
    '''
    ...
    Arg(s):
        inputs : tuple[str]
            ...
            sparse depth path at time t=-1,
            sparse depth path at time t=1,
            ground truth path at time t=0,
            ground truth path at time t=-1,
            ground truth path at time t=1,
            focal length baseline path,
            intrinsics path,
            boolean flag if set then create paths only
    Returns:
        str : output concatenated image path at time t
        str : output concatenated sparse depth path at time t
        str : output concatenated ground truth path at time t
        str : output focal length baseline path
        str : output intrinsics path
    '''

    image_curr_path, \
        image_prev_path, \
        image_next_path, \
        sparse_depth_curr_path, \
        sparse_depth_prev_path, \
        sparse_depth_next_path, \
        ground_truth_curr_path, \
        ground_truth_prev_path, \
        ground_truth_next_path, \
    ...
    if not paths_only:
        ...
        # Read sparse depth maps and concatenate together
        sparse_depth_curr = cv2.imread(sparse_depth_curr_path, cv2.IMREAD_UNCHANGED)
        sparse_depth_prev = cv2.imread(sparse_depth_prev_path, cv2.IMREAD_UNCHANGED)
        sparse_depth_next = cv2.imread(sparse_depth_next_path, cv2.IMREAD_UNCHANGED)
        sparse_depth = np.concatenate([sparse_depth_prev, sparse_depth_curr, sparse_depth_next], axis=1)

        # Read ground truth depth maps and concatenate together
        ground_truth_curr = cv2.imread(ground_truth_curr_path, cv2.IMREAD_UNCHANGED)
        ground_truth_prev = cv2.imread(ground_truth_prev_path, cv2.IMREAD_UNCHANGED)
        ground_truth_next = cv2.imread(ground_truth_next_path, cv2.IMREAD_UNCHANGED)
        ground_truth = np.concatenate([ground_truth_prev, ground_truth_curr, ground_truth_next], axis=1)

    image_output_path = sparse_depth_curr_path \
    ...
    sparse_depth_output_path = sparse_depth_curr_path \
        .replace(KITTI_DEPTH_COMPLETION_DIRPATH, KITTI_DEPTH_COMPLETION_DERIVED_DIRPATH) \
        .replace(os.path.join(os.sep + 'proj_depth', 'velodyne_raw'), '') \
        .replace('sparse_depth', 'sparse_depth_triplet')

    ground_truth_output_path = ground_truth_curr_path \
        .replace(KITTI_DEPTH_COMPLETION_DIRPATH, KITTI_DEPTH_COMPLETION_DERIVED_DIRPATH) \
        .replace(os.path.join(os.sep + 'proj_depth', 'groundtruth'), '') \
        .replace('groundtruth', 'ground_truth_triplet')
    ...
    if not paths_only:
        ...
        cv2.imwrite(sparse_depth_output_path, sparse_depth)
...
def setup_dataset_kitti_training(paths_only=False, n_thread=8):
    ...
    train_unsupervised_optical_flow_images_left_paths = []
    train_unsupervised_optical_flow_images_right_paths = []
    train_unsupervised_optical_flow_sparse_depth_left_paths = []
    train_unsupervised_optical_flow_sparse_depth_right_paths = []
    train_unsupervised_optical_flow_ground_truth_left_paths = []
    train_unsupervised_optical_flow_ground_truth_right_paths = []
    train_unsupervised_optical_flow_intrinsics_left_paths = []
    train_unsupervised_optical_flow_intrinsics_right_paths = []
    train_unsupervised_optical_flow_focal_length_baseline_left_paths = []
    train_unsupervised_optical_flow_focal_length_baseline_right_paths = []
        ...
        for sequence_dirpath in sparse_depth_sequence_dirpath:
            ...
            if refdir == 'train':
                ...
                for camera_idx, data_paths in zip([0, 1], [data_left_paths, data_right_paths]):
                    ...
                    pool_inputs = []
                    pool_optical_flow_inputs = []
                    for idx in range(len(sparse_depth_paths)):
                        # Get current, previous, and next frame for images
                        ...
                        # Get current, previous, and next frame for sparse depth maps
                        sparse_depth_curr_path = sparse_depth_paths[idx]

                        if idx > 0:
                            sparse_depth_prev_path = sparse_depth_paths[idx - 1]
                        else:
                            sparse_depth_prev_path = sparse_depth_paths[idx]  # Use current frame if no previous frame

                        if idx < len(sparse_depth_paths) - 1:
                            sparse_depth_next_path = sparse_depth_paths[idx + 1]
                        else:
                            sparse_depth_next_path = sparse_depth_paths[idx]  # Use current frame if no next frame

                        # Get current, previous, and next frame for ground truth depth maps
                        ground_truth_curr_path = ground_truth_paths[idx]

                        if idx > 0:
                            ground_truth_prev_path = ground_truth_paths[idx - 1]
                        else:
                            ground_truth_prev_path = ground_truth_paths[idx]  # Use current frame if no previous frame

                        if idx < len(ground_truth_paths) - 1:
                            ground_truth_next_path = ground_truth_paths[idx + 1]
                        else:
                            ground_truth_next_path = ground_truth_paths[idx]  # Use current frame if no next frame
                        ...
                        pool_optical_flow_inputs.append((
                            image_curr_path,
                            image_prev_path,
                            image_next_path,
                            sparse_depth_curr_path,
                    ...
                    with mp.Pool(n_thread) as pool:
                        ...
                        pool_optical_flow_results = pool.map(process_frame_optical_flow, pool_optical_flow_inputs)
                        ...
                        for optical_flow_result in pool_optical_flow_results:
                            optical_flow_images_path, \
                                optical_flow_sparse_depth_path, \
                                optical_flow_ground_truth_path, \
                                optical_flow_focal_length_baseline_path, \
                                optical_flow_intrinsics_path = optical_flow_result

                            # Save paths for left images
                            if camera_idx == 0:
                                train_unsupervised_optical_flow_images_left_paths.append(optical_flow_images_path)
                                train_unsupervised_optical_flow_sparse_depth_left_paths.append(
                                    optical_flow_sparse_depth_path)
                                train_unsupervised_optical_flow_ground_truth_left_paths.append(
                                    optical_flow_ground_truth_path)
                                train_unsupervised_optical_flow_intrinsics_left_paths.append(
                                    optical_flow_intrinsics_path)
                                train_unsupervised_optical_flow_focal_length_baseline_left_paths.append(
                                    optical_flow_focal_length_baseline_path)
                            # Save paths for right images
                            else:
                                train_unsupervised_optical_flow_images_right_paths.append(optical_flow_images_path)
                                train_unsupervised_optical_flow_sparse_depth_right_paths.append(
                                    optical_flow_sparse_depth_path)
                                train_unsupervised_optical_flow_ground_truth_right_paths.append(
                                    optical_flow_ground_truth_path)
                                train_unsupervised_optical_flow_focal_length_baseline_right_paths.append(
                                    optical_flow_focal_length_baseline_path)
                                train_unsupervised_optical_flow_intrinsics_right_paths.append(
                                    optical_flow_intrinsics_path)
    ...
    # Sort optical flow paths alphabetically
    train_unsupervised_optical_flow_images_left_paths = sorted(train_unsupervised_optical_flow_images_left_paths)
    train_unsupervised_optical_flow_images_right_paths = sorted(train_unsupervised_optical_flow_images_right_paths)
    train_unsupervised_optical_flow_sparse_depth_left_paths = sorted(
        train_unsupervised_optical_flow_sparse_depth_left_paths)
    train_unsupervised_optical_flow_sparse_depth_right_paths = sorted(
        train_unsupervised_optical_flow_sparse_depth_right_paths)
    train_unsupervised_optical_flow_ground_truth_left_paths = sorted(
        train_unsupervised_optical_flow_ground_truth_left_paths)
    train_unsupervised_optical_flow_ground_truth_right_paths = sorted(
        train_unsupervised_optical_flow_ground_truth_right_paths)
    train_unsupervised_optical_flow_focal_length_baseline_left_paths = sorted(
        train_unsupervised_optical_flow_focal_length_baseline_left_paths)
    train_unsupervised_optical_flow_focal_length_baseline_right_paths = sorted(
        train_unsupervised_optical_flow_focal_length_baseline_right_paths)
    train_unsupervised_optical_flow_intrinsics_left_paths = sorted(
        train_unsupervised_optical_flow_intrinsics_left_paths)
    train_unsupervised_optical_flow_intrinsics_right_paths = sorted(
        train_unsupervised_optical_flow_intrinsics_right_paths)

    # Combine optical flow paths for left and right images
    train_unsupervised_optical_flow_images_paths = \
        train_unsupervised_optical_flow_images_left_paths + train_unsupervised_optical_flow_images_right_paths
    train_unsupervised_optical_flow_sparse_depth_paths = \
        train_unsupervised_optical_flow_sparse_depth_left_paths + train_unsupervised_optical_flow_sparse_depth_right_paths
    train_unsupervised_optical_flow_ground_truth_paths = \
        train_unsupervised_optical_flow_ground_truth_left_paths + train_unsupervised_optical_flow_ground_truth_right_paths
    train_unsupervised_optical_flow_focal_length_baseline_paths = \
        train_unsupervised_optical_flow_focal_length_baseline_left_paths + train_unsupervised_optical_flow_focal_length_baseline_right_paths
    train_unsupervised_optical_flow_intrinsics_paths = \
        train_unsupervised_optical_flow_intrinsics_left_paths + train_unsupervised_optical_flow_intrinsics_right_paths
    '''
    ...
    # Filter clean data from left optical flow images
    train_optical_flow_data_left_paths = [
        train_unsupervised_optical_flow_images_left_paths,
        train_unsupervised_optical_flow_sparse_depth_left_paths,
        train_unsupervised_optical_flow_ground_truth_left_paths,
        train_unsupervised_optical_flow_focal_length_baseline_left_paths,
    ...
    # Filter clean data from right optical flow images
    train_optical_flow_data_right_paths = [
        train_unsupervised_optical_flow_images_right_paths,
        train_unsupervised_optical_flow_sparse_depth_right_paths,
        train_unsupervised_optical_flow_ground_truth_right_paths,
        train_unsupervised_optical_flow_focal_length_baseline_right_paths,
    ...
    # All unsupervised optical flow paths
    print('Storing {} unsupervised optical flow training images file paths into: {}'.format(
        len(train_unsupervised_optical_flow_images_paths), TRAIN_UNSUPERVISED_OPTICAL_FLOW_IMAGES_FILEPATH))
    data_utils.write_paths(TRAIN_UNSUPERVISED_OPTICAL_FLOW_IMAGES_FILEPATH,
                           train_unsupervised_optical_flow_images_paths)

    print('Storing {} unsupervised optical flow training sparse depth file paths into: {}'.format(
        len(train_unsupervised_optical_flow_sparse_depth_paths), TRAIN_UNSUPERVISED_OPTICAL_FLOW_SPARSE_DEPTH_FILEPATH))
    data_utils.write_paths(TRAIN_UNSUPERVISED_OPTICAL_FLOW_SPARSE_DEPTH_FILEPATH,
                           train_unsupervised_optical_flow_sparse_depth_paths)

    print('Storing {} unsupervised optical flow training ground truth file paths into: {}'.format(
        len(train_unsupervised_optical_flow_ground_truth_paths), TRAIN_UNSUPERVISED_OPTICAL_FLOW_GROUND_TRUTH_FILEPATH))
    data_utils.write_paths(TRAIN_UNSUPERVISED_OPTICAL_FLOW_GROUND_TRUTH_FILEPATH,
                           train_unsupervised_optical_flow_ground_truth_paths)

    print('Storing {} unsupervised optical flow training focal length and baseline file paths into: {}'.format(
        len(train_unsupervised_optical_flow_focal_length_baseline_paths),
        TRAIN_UNSUPERVISED_OPTICAL_FLOW_FOCAL_LENGTH_BASELINE_FILEPATH))
    data_utils.write_paths(TRAIN_UNSUPERVISED_OPTICAL_FLOW_FOCAL_LENGTH_BASELINE_FILEPATH,
                           train_unsupervised_optical_flow_focal_length_baseline_paths)

    print('Storing {} unsupervised optical flow training intrinsics file paths into: {}'.format(
        len(train_unsupervised_optical_flow_intrinsics_paths), TRAIN_UNSUPERVISED_OPTICAL_FLOW_INTRINSICS_FILEPATH))
    data_utils.write_paths(TRAIN_UNSUPERVISED_OPTICAL_FLOW_INTRINSICS_FILEPATH,
                           train_unsupervised_optical_flow_intrinsics_paths)
