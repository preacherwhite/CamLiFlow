    ...
    def run(self):
        ...
        for i, inputs in enumerate(self.test_loader):
            ...
            for batch_id in range(size_of_batch(inputs)):
                ...
                save_flow_png('%s/flow/%06d.png' % (out_dir, test_id), flow_2d_pred)
                
                # Save 3d flow 
                flow_3d_pred = outputs['flow_3d'][batch_id].cpu().numpy()
                os.makedirs('%s/flow3d' % out_dir, exist_ok=True)
                np.save('%s/flow3d/%06d.npy' % (out_dir, test_id), flow_3d_pred)
                ...
                os.makedirs('%s/image1' % out_dir, exist_ok=True)
                os.makedirs('%s/image2' % out_dir, exist_ok=True)
                cv2.imwrite('%s/image1/%06d.png' % (out_dir, test_id), input_image1)
                cv2.imwrite('%s/image2/%06d.png' % (out_dir, test_id), input_image2)
                ...
                os.makedirs('%s/depth1' % out_dir, exist_ok=True)
                os.makedirs('%s/depth2' % out_dir, exist_ok=True)
                cv2.imwrite('%s/depth1/%06d.png' % (out_dir, test_id), input_depth1)
                cv2.imwrite('%s/depth2/%06d.png' % (out_dir, test_id), input_depth2)
